...
def get_valid_keys_from_db(db_file: str):
    """
    Query the DB for all non-expired keys.
    Returns a list of (kid, private_key_object) tuples.
    ...
    # Parameterized query: never interpolate values directly into SQL strings
...
def get_expired_key_from_db(db_file: str):
    """
    Query the DB for one expired key.
    Returns a single (kid, private_key_object) tuple, or None if not found.
    Uses a parameterized query to prevent SQL injection.
    """
    now = int(time.time())
    conn = sqlite3.connect(db_file)
